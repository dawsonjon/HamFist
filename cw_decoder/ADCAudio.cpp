...
void ADCAudio::begin(const uint8_t audio_pin, const uint32_t audio_sample_rate)
{
  ...
  buffer_number = 0;
  dma_channel_configure(adc_dma, &cfg, samples[buffer_number], &adc_hw->fifo, ADC_DMA_SIZE, true);
}
...
int16_t* ADCAudio ::input_samples()
{
  ...
  if (duration < 10)
    Serial.println("Time Warning");

  // start a transfer into pong buffer for next time
  dma_channel_configure(adc_dma, &cfg, samples[buffer_number ^ 1], &adc_hw->fifo, ADC_DMA_SIZE,
                        true);

  // decimate down to ADC_BLOCK_SIZE samples
  for (uint16_t idx = 0; idx < ADC_BLOCK_SIZE; idx++) {
    // combine 4 samples
    int16_t sample = samples[buffer_number][idx * 4] + samples[buffer_number][idx * 4 + 1] +
                     samples[buffer_number][idx * 4 + 2] + samples[buffer_number][idx * 4 + 3];
