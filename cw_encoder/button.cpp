...
c_button :: c_button(uint8_t gpio_num):gpio_num(gpio_num)
{
    button_state = up;
    time_pressed = 0;
    gpio_init(gpio_num);
...
void c_button :: update_state()
{
  ...
  {
    if((time_us_32() - time_pressed) > (200 * 1000))
    {
      button_state = held;
    }
...
bool c_button :: is_pressed()
{
  update_state();
  return button_state == pressed;
}

bool c_button :: is_held()
{
  update_state();
